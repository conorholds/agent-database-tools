...
const { createPool, executeQuery, executeTransaction, trackMigration, isMigrationApplied, getAppliedMigrations } = require('../utils/db');
...
async function migrateCommand(projectName, migrationFile, cmdOptions = {}, cmd) {
  ...
  try {
    ...
    const migrationApplied = await isMigrationApplied(pool, migrationName);
    ...
    try {
      ...
      // Track the migration
      await trackMigration(pool, migrationName, sql);
