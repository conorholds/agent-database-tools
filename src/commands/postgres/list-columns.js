// src/commands/postgres/list-columns.js
// This file is used to list columns in a PostgreSQL table
...
/**
 * Lists columns in a PostgreSQL table
 ...
 */
async function listPostgresColumns(connection, table, options) {
  try {
    ...
    console.log(chalk.cyan(`Listing columns for table "${table}"`));
    ...
    const result = await db.postgres.executeQuery(connection, query, [table]);
    ...
    const indexResult = await db.postgres.executeQuery(connection, indexQuery, [table]);
    ...
    console.error(chalk.red('Error listing PostgreSQL columns:'), error.message);
    return false;
  }
}

module.exports = listPostgresColumns;